...
Qualifying also holds a lot of strategic importance to the race weekend, depending on the track. While there are some tracks where qualifying matters less to the race, like Max Verstappen's incredible 17th to 1st drive at the 2024 São Paulo Grand Prix, there are many examples of where it determined the winner. We will later look at a case study in Monaco, a track infamous for lacking overtaking opportunities, making qualifying all the more critical. 
...
quali_laps <- results |>
  ...
  arrange(lap_time) |>
  head()
kable(quali_laps, caption='Lap Time Data for Norris and Hülkenberg, 2024 Abu Dhabi Grand Prix.') |> kable_styling("striped", full_width = FALSE)
```
...
kable(data, caption='Results Data for Norris and Hülkenberg, 2024 Abu Dhabi Grand Prix.') |> kable_styling("striped", full_width = FALSE)
```
...
kable(plot_data, caption='Telemetry Data for Norris and Hülkenberg, 2024 Abu Dhabi Grand Prix') |> kable_styling("striped", full_width = FALSE)
```
...
The following section covers three different case studies, each examining a qualifying session that was an interesting to watch, or showcased a large difference. The first, @sec-brazil, showcases how weather affects lap times, by looking at lap times during the São Paulo Grand Prix in 2023 (dry) and 2024 (wet) for Lando Norris. The second and third case studies both look at scenarios where we compare drivers during the same session. @sec-monaco looks at lap times from the drivers who qualified first and second, to observe how important the tenths of a second are around a track like Monaco. @sec-canada takes lap times from two drivers from the same team, to highlight how even with the same machinery, driver ability plays a large part in pace and overall performance. 
...
```{r degbrazil, out.width='70%', fig.align='center', fig.cap='Plot of the seven fastest lap times and tire degredation across the two seasons.', fig.height=4}
#| echo: false
#| warning: false
#| message: false
...
With a first-lap crash and a safety car allowing drivers to change tires immediately without fear of losing strategy to another team, the 2024 Monaco Grand Prix essentially consisted of every driver going around in circles for the duration of the race. The finishing order was almost identical to the starting order (bar a few DNFs). While not so interesting to watch live, it highlights how important qualifying is to a driver's success, especially on particular tracks.

```{r degmonaco, out.width='70%', fig.align='center', fig.cap='Plot of the seven fastest lap times and tire degredation across the two drivers', fig.height=4}
#| echo: false
#| warning: false
#| message: false

quali_laps <- fetch_quali_data(2024, 8) |>
...
ggplot(quali_laps, aes(x = index, y = lap_time)) +
  ...
  coord_flip() +
  labs(
    title = "Tire Performance Degradation for Piastri and Leclerc",
    y = "Lap Time (seconds)",
    color = "Tire Compound",
    linetype = "Season"
  ) +
...
```{r degcanada, out.width='70%', fig.align='center', fig.cap='Plot of the seven fastest lap times and tire degredation across the two drivers', fig.height=4}
#| echo: false
#| warning: false
#| message: false

quali_laps <- fetch_quali_data(2024, 9) |>
...
ggplot(quali_laps, aes(x = index, y = lap_time)) +
  ...
  coord_flip() +
  labs(
    title = "Tire Performance Degradation for Verstappen and Pérez",
    y = "Lap Time (seconds)",
    color = "Tire Compound",
    linetype = "Season"
  ) +
...
arrow_start <- ver |> slice(5)
arrow_end <- ver |> slice(15)
