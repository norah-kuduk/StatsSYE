...
- F1 is an international motorsport series governed by the Fédération Internationale de l'Automobile (FIA), involving 20 drivers across 10 teams who compete in 18–24 race weekends annually

- Each weekend features practice, qualifying, and race sessions, with qualifying laps representing single-lap performance used to decide the starting order for the race. 

- This project centers on the analysis of F1 qualifying sessions, making comparisons between two drivers in the same race or the same driver across different seasons. 
...
2. Provide an interactive, visual platform for comparing qualifying laps for users.
...
- Creation of a local R package (`f1animateR`) that uses some functions from `f1dataR`, but also has functions I've created
...
## Season Comparison
First, we look at all qualifying laps and postioning in fastest laps from Lando Norris in 2023 versus 2024.
```{r tiredegplot, out.width='100%', fig.align='center', fig.cap='Plot of lap times and tire degredation across the two seasons', fig.height=4}
...
ggplot(quali_laps, aes(x = index, y = lap_time)) +
  ...
  geom_label_repel(aes(label = lap_number), nudge_x = 0.2) +
...
```
```{r resultstable, out.width='80%',fig.align='center', fig.title='Table of qualifying results for Lando Norris'}
...
```{r animsampleplot, out.width='100%', echo=FALSE, warnings=FALSE, fig.cap='Snapshot of a late point in the lap where cars are very far apart (about 64 seconds in)', fig.height=5}
...
## Driver Comparison
Second, we look at the fastest laps from Max Verstappen and Zhou Guanyu in 2023.

```{r drivercomp, out.width='80%', fig.align='center', fig.cap='table of qualifying results for Max Verstappen and Zhou Guanyu'}
...
```{r driveranimplot, out.width='100%', fig.align='center', fig.cap='Snapshot of a late point in the lap where the two drivers\' cars are far apart (about 69 seconds in)'}
...
- Implementing a feature that allows user to select which lap number they'd like to visualize. 
- Utilize sector times more in analysis and eventually expland to race simulations.

# Conclusion

Data is a very large part of Formula 1, cisual analytics offer deeper insight into qualifying performance for the average viewer. 
