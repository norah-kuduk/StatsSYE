...
#' @param grouping A variable containing either `driver` or `season` to group speed by
...
#' @export
smooth_speed <- function(plot_data, grouping, frame_size = 16) {
  if (grouping == "driver") {
    plot_data |>
      group_by(driver) |>
      # create groups of 4
      mutate(group = ceiling(row_number() / frame_size)) |> # why cant i put frame size here
